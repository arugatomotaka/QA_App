...
import jp.techacademy.tomotaka.aruga.qa_app.databinding.ActivityQuestionSendBinding
...
//！= →等しくない！！
        ...
        // URIからBitmapを取得する
        //Bitmapクラスは画像を表示したり変更したりすることができる
        val image: Bitmap
        try {
    ...
    override fun onClick(v: View) {
        if (v === binding.imageView) {
            ...
            //↑ContentsPATHというルートノードの下にある、genreという整数値を文字列に変換したノードへのリファレンスを取得しています。

            val data = HashMap<String, String>()

            // ↓Firebase Authenticationを使用して現在ログインしているユーザーのUID（ユーザーID）を取得し、それをdataというマップに追加するためのもの。
            ...
            if (title.isEmpty()) {
                ...
                return
                //↑のreturnは、emptyの場合これ以上処理を続行させたくないから、returnを用いて関数の実行を終了させている
            }
